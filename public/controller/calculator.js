...
app.controller('calculatorController', ['$scope', '$timeout', '$interval', "$http", function ($scope, $timeout, $interval, $http) {
    ...
    $scope.calculate = function (e, value) {
        ...
        if(e === 2){
            ...
            $scope.isYear = false
            $scope.isActive = true
            $scope.message = "El servicio Preventico consta de 3 erogaciones programadas (vease el protogolo de prevencion de prockeep el modelo BIO TEC Ambiente Seguro)"
           tsm = parseInt(value) * 2.5 
           // PRECIO 1
           if(tsm >= 50 && tsm <= 120){
                $scope.one = 50
